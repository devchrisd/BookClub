...
class BookRepository extends EntityRepository {
    ...
    public function getBookSummary()
    {
        ...
        $q = $em->createQuery('select count(b.id) bc, sum(b.kword) wc, sum(b.page) pc from AppBundle:BookBook b');
    ...
    {
        $em = $this->getEntityManager();
        $q = $em->createQuery('select v.visitwhen from AppBundle:BookVisit v where v.bid=:bid order by v.visitwhen desc');
    ...
    {
        $em = $this->getEntityManager();
        $q = $em->createQuery('select count(v.bid) vc from AppBundle:BookVisit v where v.bid=:bid');
        $q->setParameter('bid', $bid);
        ...
        if ($count != 0)
        {
            $rec = new \AppBundle\Entity\BookVisit();
            $rec->setBid($bid);
    ...
    public function getResultAndCount($page, $rpp, $key=null)
    {
        ...
        if ($key == 'null')
        {
            $q1 = $em->createQuery('select count(b.id) bc from AppBundle:BookBook b');
        }
        else
        {
            $qstr = sprintf("select count(b.id) bc from AppBundle:BookBook b where b.title like '%s%%'", $key);
    ...
    {
        $repo = $this->getEntityManager()->getRepository('AppBundle:BookTaglist');
    ...
    public function getTagResultAndCount($page, $rpp, $tag=null)
    {
        // Get the book count
        $em = $this->getEntityManager();
        if ($tag == 'null')
        {
            $q1 = $em->createQuery('select count(b.tid) bc from AppBundle:BookTaglist b');
        }
        else
        {
            $qstr = sprintf("select count(b.tid) bc from AppBundle:BookTaglist b where b.tag='%s'", $tag);
            $q1 = $em->createQuery($qstr);
        }
        $res1 = $q1->getSingleResult();
        $count = $res1['bc'];
        ...
        // Now get the wanted result specified by page
        $repo = $em->getRepository('AppBundle:BookTaglist');
        $q2 = $repo->createQueryBuilder('t')
                // ->select('t', 'b')
                // ->join('AppBundle:BookTaglist', 't', 'WITH', 't.bookid=b.id')
                ->setMaxResults($rpp)
                ->setFirstResult(($page - 1) * $rpp)
                ->orderBy('t.tid', 'desc');
        if ($tag <> 'null')
