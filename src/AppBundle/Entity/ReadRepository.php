...
class ReadRepository extends EntityRepository
{
    public function getReadSummary()
    {
        ...
        $q=$em->createQuery('select count(r.id) rc from AppBundle:BookReview r');
    ...
    public function getResultAndCount($page, $rpp)
    {
        // Get the book count
        $em = $this->getEntityManager();
        $q1 = $em->createQuery('select count(r.id) rc from AppBundle:BookHeadline r');
    ...
    {
        $repo=$this->getEntityManager()->getRepository('AppBundle:BookReview');
        $q=$repo->createQueryBuilder('r')
